...
// 3. 把 /srv/code/images/lang-python26.tar.gz 解壓縮進 chroot 中
...
class Prebuilder
{
    ...
    public function main($req_file, $force_rebuild = false)
    {
        ...
        system("tar zxf /srv/code/images/lang-python26.tar.gz --directory={$root}");

        error_log('find diff file...');
        // 把新解進去的檔案都改成 2001/1/1
        system("find {$root} -printf '%TY%Tm%Td %p\n' | grep -v ',,,/proc' | grep -v '^20000101' | awk '{print $2}' | xargs -n 100 touch --no-dereference --date=20000101");
